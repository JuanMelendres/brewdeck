...
1. Slice B.2 — per-user read-filtering: scope collection GETs / analytics to the current user's owned rows, then enforce owner_id NOT NULL once every row is guaranteed an owner. (B.1 owner columns + write-path stamping shipped.)
